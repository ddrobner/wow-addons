...
local MacroTooltip = CreateFrame("GameTooltip", nil, UIParent) do
...
local mainPanel = CreateFrame("Frame", "M6UI", UIParent, "PortraitFrameTemplate") do
	local f = mainPanel do
		f:SetWidth(578)
		f:Hide()
		SetPortraitToTexture(f.portrait, "Interface\\icons\\secrets_of_the_empire_icon")
		...
		f.Bg1 = f:CreateTexture(nil, "BACKGROUND", nil, 1)
		...
		f.BgSep:SetPoint("TOPRIGHT", f.Bg1, "BOTTOMRIGHT", 2, 1)
		f.BgSep:SetHeight(8)
		...
		do -- Move Handles
			...
			h:SetPoint("BOTTOMRIGHT", f.TitleText, "BOTTOMRIGHT", 28, -6)
			h:SetScript("OnMouseDown", Mover_OnDown)
			h:SetScript("OnMouseUp", Mover_OnUp)
		end
	...
	local sf, bar, slate = CreateFrame("ScrollFrame", nil, contentSlot) do
		...
		bar = CreateFrame("Slider", nil, sf) do
			bar:SetWidth(19)
			bar:SetPoint("TOPLEFT", sf, "TOPRIGHT", 2, -11)
			bar:SetPoint("BOTTOMLEFT", sf, "BOTTOMRIGHT", 2, 13)
	...
	local editPanel = CreateFrame("Frame", nil, contentSlot) do
		...
		local bind = CreateFrame("Button", nil, editPanel, "UIPanelButtonTemplate") do
			...
			local glob = CreateFrame("CheckButton", nil, bind, "InterfaceOptionsCheckButtonTemplate") do
				glob:SetHitRectInsets(0,0,0,0)
				glob:SetPoint("LEFT", bind, "RIGHT", 2, 0)
				glob:SetScript("OnEnter", function(self)
					GameTooltip:SetOwner(self, "ANCHOR_TOP")
					GameTooltip:SetText(L"Global binding")
				end)
				glob:SetScript("OnLeave", function(self)
					if GameTooltip:IsOwned(self) then
						GameTooltip:Hide()
					end
				end)
		...
		save:SetScript("OnClick", function() editPanel:Save() end)
	...
	local menu, drop = {
		{text="Place on action bars", func=function(_, id) M6:PickupAction(id) end, notCheckable=true},
		{text="Deactivate", notCheckable=true, func=function(_, id) M6:DeactivateAction(id) wipe(slate.keys) sf:Reset() end},
		{text="|cffff0000Delete", notCheckable=true, func=function(_, id) M6:DeleteAction(id) wipe(slate.keys) sf:Reset() end},
	}, CreateFrame("Frame", "M6ActionsDropDown", slate, "UIDropDownMenuTemplate")
	local function Button_OnClick(self, button)
		local id = self:GetID()
		if id == 0 or button == "LeftButton" then
			editPanel:Open(self:GetID())
	...
	function slate:DoLayout(ofs, h)
		...
		for i=1,#kt do
			...
			sb:SetID(id)
			local ico, showBorder = "Interface/GuildBankFrame/UI-GuildBankFrame-NewTab", false
			if id > 0 then
				ico = M6:GetActionIcon(id)
				if not ico then
				...
				if M6:IsActionActivated(id) then
					sb.border:SetAtlas("loottoast-itemborder-blue")
					showBorder = true
				elseif M6:GetActionBind(id) then
					sb.border:SetAtlas("loottoast-itemborder-green")
					showBorder = true
				end
			end
			sb.border:SetShown(showBorder)
			sb.Icon:SetTexture(ico)
			sb:Show()
		end
		for i=#kt+1,#bt do
			bt[i]:Hide()
...
SLASH_M61, SlashCmdList.M6 = "/m6", function(arg)
	if arg == "vers" then
		print(("|cffa526ff[%s] |cffffffff%s|cfff8a800%s |cffa0a0a0(%s)"):format(
			addonName, GetAddOnMetadata(addonName, "Version") or "?",
			T.SkipLocalActionBook and "+sha" or "+int",
			GetAddOnMetadata(addonName, "X-Version-Tag") or "?T"
		))
		return
	end
